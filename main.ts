...
function attemptJump () {
    ...
    if (protagonista.isHittingTile(CollisionDirection.Bottom)) {
        protagonista.vy = -4 * pixelsToMeters
    } else if (canDoubleJump) {
        doubleJumpSpeed = -3 * pixelsToMeters
        // Good double jump
        if (protagonista.vy >= -40) {
            doubleJumpSpeed = -4.5 * pixelsToMeters
            protagonista.startEffect(effects.trail, 500)
            scene.cameraShake(2, 250)
        }
        protagonista.vy = doubleJumpSpeed
        canDoubleJump = false
    }
}
function animateIdle () {
    mainIdleLeft = animation.createAnimation(ActionKind.Walking, 100)
    animation.attachAnimation(protagonista, mainIdleLeft)
    ...
    animation.attachAnimation(protagonista, mainIdleRight)
...
function animateRun () {
    ...
    animation.attachAnimation(protagonista, mainRunLeft)
    ...
    animation.attachAnimation(protagonista, mainRunRight)
...
function animateJumps () {
    ...
    animation.attachAnimation(protagonista, mainJumpLeft)
    ...
    animation.attachAnimation(protagonista, mainJumpRight)
...
function animateCrouch () {
    ...
    animation.attachAnimation(protagonista, mainCrouchLeft)
    ...
    animation.attachAnimation(protagonista, mainCrouchRight)
...
function initializeLevel (level: number) {
    ...
    tiles.placeOnTile(protagonista, playerStartLocation)
...
protagonista = sprites.create(assets.image`protagonista`, SpriteKind.Player)
...
game.onUpdate(function () {
    ...
    if (protagonista.isHittingTile(CollisionDirection.Top)) {
        protagonista.vy = 0
    }
    if (controller.down.isPressed()) {
        if (heroFacingLeft) {
            animation.setAction(protagonista, ActionKind.Walking)
        } else {
            animation.setAction(protagonista, ActionKind.Walking)
        }
    } else if (protagonista.vy < 20 && !(protagonista.isHittingTile(CollisionDirection.Bottom))) {
        if (heroFacingLeft) {
            animation.setAction(protagonista, ActionKind.Walking)
        } else {
            animation.setAction(protagonista, ActionKind.Walking)
...
game.onUpdate(function () {
    for (let value8 of sprites.allOfKind(SpriteKind.Flier)) {
        if (Math.abs(value8.x - protagonista.x) < 60) {
            if (value8.x - protagonista.x < -5) {
                value8.vx = 25
            } else if (value8.x - protagonista.x > 5) {
                value8.vx = -25
            }
            if (value8.y - protagonista.y < -5) {
                value8.vy = 25
            } else if (value8.y - protagonista.y > 5) {
                value8.vy = -25
            }
